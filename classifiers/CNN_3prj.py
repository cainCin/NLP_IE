...
ENC = W2VEncoder(weight_paths=r"D:\Workspace\cinnamon\code\github\NLP_IE\weights\jawiki_20180420_100d.pkl")
encode_length = 100
_default = {
    "lr": 1e-4,
    "weight_path": "W2VCNN_test_3prj_1e4.pt",
}
...
def transform(text, encode_engine=ENC, max_length=max_length):
    ...
    try:
        ...
        print(f"DEBUG: Error in encoding {text}")
    else:
        out[:, :enc.shape[1],:] = enc
    finally:
        return out

category = ['', 'address', 'company_name', 'date', 'description', 'number', 'person_name', 'tel_fax', 'zipcode']
...
def train(net, trainloader, testloader, transform=None, target_transform=None, device=torch.device("cpu"), config=_default, only=["value"]):
...
if __name__ == "__main__":
    ...
    project_test = [
        ...
        {
            "name": "bourbon",
            "label_path": r"D:\Workspace\cinnamon\data\Bourbon\val\labels",
            "mapping_func": mapping_list["bourbon"],
        },
        {
            "name": "sompo_holdings",
    ...
    train(net, trainloader, testloader, transform=transform, target_transform=target_transform, device=device, config=_default, only=["value"])
    # load checkpoint
    #net.load_state_dict(torch.load(_default.get('weight_path')))
    #net.eval()
    #print(f"Load {_default.get('weight_path')}...")
